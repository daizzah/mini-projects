...
class MathSolverApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.iconbitmap("asdf.ico")
        self.root.title("Math Solver")
        self.root.geometry("600x500")

        # Create a symbol for algebraic expression
        self.x = sp.Symbol('x')
        self.current_equation = None

        # Create notebook and add tabs
        ...
        self.style.configure(
            ...
            borderwidth=2,
            relief="flat",
            background="#f2c6de",  # light pink
            foreground="#3c2c66"  # deep purple
        )
        self.style.map("Pink.TButton",
            background=[("active", "#eab4cc"), ("!disabled", "#f2c6de")],
            foreground=[("active", "#3c2c66")]
        )
    ...
    def styled_container(self, tab):
        container = ttk.Frame(tab, style="TFrame")  # Use ttk frame with the style
    ...
    def styled_label(self, container, text, size=16, bold=False):
        ...
        if not self.style.lookup(style_name, "font"):
            # Inherit layout from base TLabel (this line prevents the TclError!)
    ...
    def styled_button(self, container, text, command, color="info"):
        return Button(
            container,
            text=text,
            command=command,
            bootstyle=f"{color}-outline rounded-pill",  # cute + outline
            padding=(16, 10)
        )

    def rounded_button_wrapper(self, container, button):
        wrapper = tk.Frame(container, bg="#3c2c66")  # outline color
        wrapper.pack(pady=10)

        inner = tk.Frame(wrapper, bg="#f2c6de")  # match button bg
        inner.pack(padx=2, pady=2)  # padding = border thickness

        button.configure(style="Pink.TButton")
        button.pack(in_=inner)
    ...
    def setup_algebra_tab(self):
        ...
        self.rounded_button_wrapper(container, self.algebra_generate_button)
        ...
        self.rounded_button_wrapper(container, self.algebra_solve_button)
    ...
    def setup_trig_tab(self):
        ...
        self.trig_generate_button = self.styled_button(container, "🎲 Generate Another Problem", self.generate_trig, "#b28dff")
        self.rounded_button_wrapper(container, self.trig_generate_button)

        self.trig_question_label = self.styled_label(container, "", size=16)
        self.trig_question_label.pack(pady=10)

        self.trig_solve_button = self.styled_button(container, "✅ Solve", self.solve_trig, "#9472db")
        self.rounded_button_wrapper(container, self.trig_solve_button)
    ...
    def setup_geo_tab(self):
        ...
        self.geo_generate_button = self.styled_button(container, "🎲 Generate Another Problem", self.generate_geometry, "#b28dff")
        self.rounded_button_wrapper(container, self.geo_generate_button)

        self.geo_question_label = self.styled_label(container, "", size=16)
        self.geo_question_label.pack(pady=10)

        self.geo_solve_button = self.styled_button(container, "✅ Solve", self.solve_geometry, "#9472db")
        self.rounded_button_wrapper(container, self.geo_solve_button)
